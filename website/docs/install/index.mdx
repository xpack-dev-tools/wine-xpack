...
The command to install the **latest** available version of **wine** is:
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/wine@${customField('releaseVersion')} --verbose
`} </CodeBlock>
...
In order to access the WineHQ binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/wine/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<ul>
<li><code>xpack-wine-{customField('xpackVersion')}-linux-x64.tar.gz</code></li>
...
example <code>/home/ilg/.local/xPacks/wine/xpack-wine-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/wine
cd ~/.local/xPacks/wine

tar xvf ~/Downloads/xpack-wine-${customField('xpackVersion')}-linux-x64.tar.gz
chmod -R -w xpack-wine-${customField('xpackVersion')}
`} </CodeBlock>
