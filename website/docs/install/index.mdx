...
The easiest (and recommended) way to install xPack WineHQ
...
* add <code>@xpack-dev-tools/wine</code> to
  `package.json` as a
  development dependency; this associates a specific version of xPack WineHQ with
...
it is also possible to install xPack WineHQ only globally in
...
In order to access the xPack WineHQ binaries installed in
...
<ul>
<li><code>xpack-wine-9.0.0-1-linux-x64.tar.gz</code></li>
</ul>
...
example <code>/home/ilg/.local/xPacks/wine/xpack-wine-9.0.0-1</code>:
