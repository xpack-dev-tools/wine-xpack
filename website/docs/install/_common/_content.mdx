...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/wine@10.0.0-1.1 --verbose
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/wine/10.0.0-1.1/.content/bin:$PATH
`} </CodeBlock>
...
<ul>
<li><code>xpack-wine-10.0.0-1-linux-x64.tar.gz</code></li>
</ul>
...
example <code>/home/ilg/.local/xPacks/wine/xpack-wine-10.0.0-1</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/wine
cd ~/.local/xPacks/wine

tar xvf ~/Downloads/xpack-wine-10.0.0-1-linux-x64.tar.gz
chmod -R -w xpack-wine-10.0.0-1
`} </CodeBlock>
