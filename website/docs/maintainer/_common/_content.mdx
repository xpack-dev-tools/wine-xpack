...
**@xpack-dev-tools/wine** is an xpm package that packages platform-specific archives, built with the [xPack Build Box (XBB)](https://xpack.github.io/xpack-build-box/).
...
(such as **10.0.0-3.1**) with the following expression:
...
npm --prefix ~/Work/xpack-dev-tools/wine-xpack.git/build-assets install
xpm install -C ~/Work/xpack-dev-tools/wine-xpack.git/build-assets
...
- In `CHANGELOG.md`, add the release date and a message
  such as _**\* v10.0.0-3 released**_
...
tagged such as **v10.0.0-3** (mind the dash in the middle!) and
named such as **xPack WineHQ v10.0.0-3** (mind the dash),
...
- **Commit** all changes with the message
  _**package.json: update URLs for 10.0.0-3.1 release**_
...
- Navigate to the [GitHub Releases](https://github.com/xpack-dev-tools/wine-xpack/releases) page
...
npm --prefix ~/Work/xpack-dev-tools/wine-xpack.git/build-assets install && \
xpm install -C ~/Work/xpack-dev-tools/wine-xpack.git/build-assets && \
xpm run test-release -C ~/Work/xpack-dev-tools/wine-xpack.git/build-assets
```
