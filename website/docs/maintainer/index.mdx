...
named like **xPack WineHQ v{customField('xpackVersion')}** (mind the dash),
...
npm dist-tag add @xpack-dev-tools/wine@${customField('xpackVersion')}.${customField('npmSubversion')} latest
npm dist-tag ls @xpack-dev-tools/wine
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`npm unpublish @xpack-dev-tools/wine@${customField('xpackVersion')}.${customField('npmSubversion')}
`} </CodeBlock>
...
- paste the release name like _**xPack WineHQ v{customField('xpackVersion')} released**_
