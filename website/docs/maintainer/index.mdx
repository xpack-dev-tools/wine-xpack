...
- determine the version (like <code>9.0.0-1.1</code>)
- update the version in top `package.json`
- use the new version, suffixed by `.pre`,
like <code>9.0.0-1.1.pre</code>.
...
Determine the version (like <code>9.0.0-1</code>)
and update the `build-assets/scripts/VERSION` file; the format
is <code>9.0.0-1</code>.
...
and fix them; assign them to a milestone (like <code>9.0.0-1</code>).
...
- add a new entry like _**\* v9.0.0-1 prepared**_
- commit with the message _**prepare v9.0.0-1**_
...
(like **9.0**) with the following expression:
...
```sh
xpm run generate-workflows -C ~/Work/xpack-dev-tools/wine-xpack.git/build-assets
```
...
```sh
xpm run trigger-workflow-build-linux-x64 -C ~/Work/xpack-dev-tools/wine-xpack.git/build-assets
```
...
[GitHub Actions](https://github.com/xpack-dev-tools/wine-xpack/actions) page.
...
watching the wine-xpack project.
...
```sh
xpm run generate-website-blog-post -C ~/Work/xpack-dev-tools/wine-xpack.git/build-assets
```

- edit the front matter properties
- select the correct summary
- commit with the message _**website: blog post release 9.0.0-1 published**_
...
- commit all changes with the message
  _**package.json: update urls for 9.0.0-1.1 release**_

### Publish on the npmjs.com server

- select the `xpack-development` branch
- check the latest commits `xpm run git-log`
- update `CHANGELOG.md`, add a line like _**\* v9.0.0-1.1 published on npmjs.com**_
- commit with the message _**CHANGELOG: publish npm v9.0.0-1.1**_
...
<a href="https://www.npmjs.com/package/@xpack-dev-tools/wine?activeTab=versions">npmjs.com</a>.
...
- switch to https://xpack-dev-tools.github.io/wine-xpack/docs/maintainer/#update-the-xpack-branch
...
```sh
xpm run trigger-workflow-deep-clean -C ~/Work/xpack-dev-tools/wine-xpack.git/build-assets
```
...
- paste the release name like _**xPack WineHQ v9.0.0-1 released**_
...
[![npm (scoped)](https://img.shields.io/npm/v/@xpack-dev-tools/wine)](https://www.npmjs.com/package/@xpack-dev-tools/wine/)
[![npm](https://img.shields.io/npm/dm/@xpack-dev-tools/wine)](https://www.npmjs.com/package/@xpack-dev-tools/wine/)
  - all releases [![npm](https://img.shields.io/npm/dt/@xpack-dev-tools/wine)](https://www.npmjs.com/package/@xpack-dev-tools/wine/)
